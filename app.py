...
def handle_postback(sender_id: str, postback: dict):
    ...
    responses = {
        ...
        "MOTORBIKE": "🛵 Thuê xe ga: 150k/ngày (24 tiếng)\nCó xe Honda Vision hoặc Airblade\n\nLiên hệ: 083 285 0488 để đặt trước nhé!",
        "CONTACT":   "📞 Hotline: 083 285 0488\n🏠 Địa chỉ: 17/14B Lương Văn Năm, Phan Thiết\n\nGọi hoặc nhắn tin bất cứ lúc nào, mình luôn sẵn sàng hỗ trợ! 😊",
    ...
    if reply:
        ...
        reply = ask_gemini(sender_id, postback.get("title", payload))
        send_text(sender_id, reply)
...
def ask_gemini(sender_id: str, user_text: str) -> str:
    ...
    # Thử tối đa 3 lần, mỗi lần chờ 5 giây nếu bị rate limit
    for attempt in range(3):
        try:
            ...
            if resp.status_code == 429:
                print(f"⏳ Rate limit, chờ 5 giây... (lần {attempt+1})")
                time.sleep(5)
                continue
