...
- Nếu khách muốn xác nhận đặt phòng, nhờ để lại SĐT để chủ nhà liên hệ lại
...
def check_room_availability(checkin: str, checkout: str) -> str:
    try:
        ...
        booked_tc = sum(1 for b in bookings if b.get("room_type_id") == ROOM_TIEU_CHUAN)
        booked_cc = sum(1 for b in bookings if b.get("room_type_id") == ROOM_CAO_CAP)
