...
- CHỈ áp dụng khi khách hỏi RÕ RÀNG về việc gửi hành lý / để đồ / gửi đồ trước check-in
- KHÔNG áp dụng khi khách hỏi còn phòng, giá phòng, hoặc các câu hỏi khác
- Nếu khách hỏi gửi hành lý mà CHƯA cọc hoặc chưa tới ngày check-in → trả lời: "Được bạn, bạn đến gửi hành lý trong phòng lễ tân sau 7h sáng nha."
- Nếu khách đã cọc VÀ hôm nay đúng là ngày check-in → trả lời: "Bạn cứ nhập mã cổng xong gửi hành lý trong phòng lễ tân nha, khi vào trong không đóng cổng vì cổng tự đóng."
...
def handle_message(sender_id: str, message: dict):
    ...
    # Xác định loại phòng khách chọn (dùng chung bên dưới)
    ...
    if any(kw in lower for kw in DAT_PHONG_KEYWORDS):
        if not checkin_s or not checkout_s:
